...
def jacobian(
    ...
    if mode == 'backward':
        grad_model = bst.augment.jacrev(fn, argnums=0, unit_aware=True)
    elif mode == 'forward':
        grad_model = bst.augment.jacfwd(fn, argnums=0, unit_aware=True)
    else:
        raise ValueError('Invalid mode. Choose between backward and forward.')
    return bst.augment.vmap(grad_model)(xs)
