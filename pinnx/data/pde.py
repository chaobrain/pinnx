...
from pinnx._model import Model
from pinnx.geometry import Geometry, GeometryXTime, AbstractGeometry
...
class PDE(Data):
    ...
    def __init__(
        ...
        self.anchors = None if anchors is None else anchors.astype(bst.environ.dftype())
    ...
    def losses(self, targets, outputs, loss_fn, inputs, model: Model, aux=None):
        f = []
        if self.pde is not None:
            f = self.pde(partial(model.fn_outputs, True), inputs)
            if not isinstance(f, (list, tuple)):
                f = [f]

        if not isinstance(loss_fn, (list, tuple)):
            loss_fn = [loss_fn] * (len(f) + len(self.ic_bcs))
        elif len(loss_fn) != len(f) + len(self.ic_bcs):
            raise ValueError("There are {} errors, but only {} losses.".format(len(f) + len(self.ic_bcs), len(loss_fn)))

        bcs_start = np.cumsum([0] + self.num_bcs)
        bcs_start = list(map(int, bcs_start))
        error_f = [fi[bcs_start[-1]:] for fi in f]
        losses = [loss_fn[i](u.math.zeros_like(error), error) for i, error in enumerate(error_f)]
        for i, bc in enumerate(self.ic_bcs):
            beg, end = bcs_start[i], bcs_start[i + 1]
            # The same BC points are used for training and testing.
            error = bc.error(self.train_x, inputs, outputs, beg, end, partial(model.fn_outputs, True))
            losses.append(loss_fn[len(error_f) + i](u.math.zeros_like(error), error))
        return losses
    ...
    def add_anchors(self, anchors):
        ...
        """
        anchors = anchors.astype(bst.environ.dftype())
    ...
    def replace_with_anchors(self, anchors):
        ...
        """
        self.anchors = anchors.astype(bst.environ.dftype())
    ...
    def train_points(self):
        X = np.empty((0, self.geom.dim), dtype=bst.environ.dftype())
        ...
        if self.num_boundary > 0:
            if self.train_distribution == "uniform":
                ...
                tmp = self.geom.random_boundary_points(self.num_boundary, random=self.train_distribution)
            X = np.vstack((tmp, X))
    ...
    def bc_points(self):
        ...
        self.train_x_bc = (
            np.vstack(x_bcs)
            if x_bcs
            else np.empty([0, self.train_x_all.shape[-1]], dtype=bst.environ.dftype())
        )
    ...
    def train_points(self):
        ...
        if self.num_initial > 0:
            if self.train_distribution == "uniform":
                ...
                tmp = self.geom.random_initial_points(self.num_initial, random=self.train_distribution)
            if self.exclusions is not None:
                def is_not_excluded(x):
                    return not np.any([np.allclose(x, y) for y in self.exclusions])
